...
# --- AYARLAR ---
st.set_page_config(page_title="Rise Farm (V56 Reset)", layout="wide", page_icon="♻️")
GB_FIYATI_TL = 360.0
...
def get_data(username):
    # Cache'i kaldırdım, her seferinde taze veri çeksin (Hata riskini sıfırlar)
    try:
        sh = get_google_sheet()
...
def save_entry_cloud(username, tarih, kategori, alt_kategori, esya, adet, fiyat, notlar):
    ...
    # KESİN HESAP: (Coin / 100 Milyon) * 360
...
def upload_json_prices(json_file):
    try:
        ...
        current_db = BASE_DB.copy() # İskeletten kopya al
        
        # Fiyatları eşle
        for cat in data:
            # JSON'daki kategorileri kontrol etmeden iskelete uydur
            if cat in current_db:
                for sub in data[cat]:
                    if sub in current_db[cat]:
                        for item, price in data[cat][sub].items():
                             # Item iskelette varsa fiyatını al
...
# --- SİLME VE GÜNCELLEME FONKSİYONLARI ---
...
def full_system_reset():
    # TÜM SEKMELERİ SİLİP YENİDEN OLUŞTURUR (HERKES İÇİN SIFIRLAMA)
    sh = get_google_sheet()
    try: sh.del_worksheet(sh.worksheet("Logs"))
    except: pass
    try: sh.del_worksheet(sh.worksheet("Prices"))
    except: pass
    try: sh.del_worksheet(sh.worksheet("Periods"))
    except: pass
    init_sheets()
    return True
...
if check_login():
    ...
    # CACHE RESET BUTONU (Çok Önemli)
    if st.sidebar.button("⚠️ SİSTEMİ SIFIRLA VE ONAR"):
        st.cache_data.clear()
        st.cache_resource.clear()
        full_system_reset()
        st.success("Sistem fabrika ayarlarına döndü. Fiyat dosyanızı tekrar yükleyin."); time.sleep(2); st.rerun()
    ...
    # --- SAYFA: ANALİZ ---
        ...
        if not df.empty:
            with st.expander("🔍 Filtrele", expanded=True):
                c1, c2, c3 = st.columns(3)
                d_fil = c1.selectbox("Dönem", ["Tüm Zamanlar", "Bugün", "Son 7 Gün", "Bu Ay"] + ([f"👑 {p}" for p in PERIOD_DB] if PERIOD_DB else []))
